...
func connectBroker() {
	...
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*300)
	c.ReportFulcrum(ctx, &pb.None{})
	cancel()
}

func askFulcrumChanges(port string) (log string, clock []byte) {
	// Set up a connection to the server.
	fmt.Println("Conectando a fulcrum " + port)
	conn, err := grpc.Dial(port, grpc.WithInsecure(), grpc.WithBlock())
	if err != nil {
...
func propagateChanges() {
	for range time.Tick(time.Second * 10) {
		...
		//replica 2
		log2, clock2 := askFulcrumChanges(":" + strconv.Itoa(base_port+1))
		...
		//replica 1
		log1, clock1 := askFulcrumChanges(":" + strconv.Itoa(base_port+1))
		...
		os.Remove("2.txt")
		// send new clocks
		sendNewClocks(":" + strconv.Itoa(base_port+1))
		sendNewClocks(":" + strconv.Itoa(base_port+2))
	}
}
...
func main() {
	...
	port := ":" + strconv.Itoa(base_port+replica_id)
	connectBroker()
