...
func (s *server) UpdateRebels(ctx context.Context, register *pb.Register) (*pb.Reply, error){
	fmt.Print("Modificando ciudad... ")
...
func connectNameNode() (port string) {
	...
	conn, err := grpc.Dial(namenode_address, grpc.WithInsecure(), grpc.WithBlock())
	if err != nil {
...
func main(){
	// port := connectNameNode()
	// listenRequests(port)foo
